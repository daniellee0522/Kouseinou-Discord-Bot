...
def lock_channel(channel_id):
    with open (config.DATA_DIR / 'lock.json','r', encoding = 'utf8') as file:
        data=json.load(file)
    if channel_id not in data:
        data.append(channel_id)
        with open(config.DATA_DIR / 'lock.json','w',encoding='utf8') as f:
...
def block_check(channel_id):
    with open (config.DATA_DIR / 'block.json','r', encoding = 'utf8') as file:
        data = json.load(file)
    if channel_id in data:
        return True
    else:
